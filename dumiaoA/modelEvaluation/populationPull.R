...
featuresWideU[!is.na(createtime.1), hightRiskTransAvg6:=as.numeric(hightRiskTransAvg6Total.1)/(as.numeric(useCardNumPost6)+0.00000000001)]
...
featuresWideU[!is.na(createtime.1), useCardPM:=useCardNumPMTotal.1/(as.numeric(useCardNumPost6)+0.00000000001)] #这个变量根本没出现6，可是却是6个月平均刷卡消费频率
...
featuresWideU<-merge(featuresWideU, tongdun[, c("financingprojectid","rule_name","final_score"), with=F], by="financingprojectid")

featuresWideU[, c("Loan_Date", "createtime"):=NULL]
###################################################################################
trainData <- featuresWideU[flgTest==0,]
testData <- featuresWideU[flgTest==1,]
# 删掉一些缺失太多的
